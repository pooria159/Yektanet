...
func generateRawEventLink(action string, selectedAd FetchedAd, requestingPublisherId int) string {
	var builder strings.Builder
	builder.Reset()
...
func generateAndSignEventLink(action string, selectedAd FetchedAd, requestingPublisherId int) (string, error) {
	var url Url
	url.Link = generateRawEventLink(action, selectedAd, requestingPublisherId)
	url.StandardClaims.IssuedAt = time.Now().Unix()
...
func makeResopnse(selectedAd FetchedAd, requestingPublisherId int) (ResponseInfo, error) {
	...
	response.ClickLink, err			= generateAndSignEventLink("click", selectedAd, requestingPublisherId)	
	if err != nil {
		return response, err
	}
	response.ImpressionLink, err	= generateAndSignEventLink("impression", selectedAd, requestingPublisherId)
	if err != nil {
		return response, err
	}
	return response, nil
...
/* Signs the generated impression and click link
 with AdServer's internal private key, so that
 it will be shown that it is really generated by AdServer. */
func signUrl(url *Url) (string, error) {
	token := jwt.NewWithClaims(jwt.SigningMethodHS256, url)
